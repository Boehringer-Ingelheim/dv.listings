...
#### Open questions
- Is it possible to have more than one reviewer per role?
...
## Storage
Currently, the two available forms of storage on Copernicus are:
...
#### Open questions
- What are the properties of the Copernicus Connect bookmark folders? Are they backed up? Do we have a storage limit?
- Will separate company mount points become available at some point on Copernicus?
...
- `ae_000.base` (first known "ae" dataset):
  ...
  - 1 per "ae" row: `hash(ae[id_vars])` + `hash(ae[tracked_vars])`
- `ae_001.delta` (one per domain update):
  - 1 file magic code ("LISTDELT")
  - 1 format version number (0)
  - 1 domain string ("ae")
  - 1 generation marker (1)
  - 1 delta timestamp (number of milliseconds ellapsed since "base" timestamp)
  - 1 per *new/modified* "ae" row: `hash(ae[id_vars])` + `hash(ae[tracked_vars])`
- `ae_<ROLE>.review` (one per <ROLE>):
  - 1 file magic code ("LISTREVI")
  - 1 format version number (0)
  - 1 role string
  - n reviews: `hash(ae[id_vars])` + `review_index` + delta timestamp
- `ae_<ROLE>.codes` (one per <ROLE>):
  - 1 file magic code ("LISTCODE")
  - 1 format version number (0)
  - n review texts

The dominant factor governing the size of these files is the length of a hash, which is 16 bytes, as we discuss in the "Hashing" session below. Delta timestamps can be encoded in 8 bytes. Estimating an upper bound of 1 million rows per dataset, a `.base` file would take around 32 MiB. A comprehensive `.review` file for such a dataset would take around 24 MiB.

These file structures are designed so that they start with a short heterogeneous header that reiterates the information that can be gleaned on the file name. The rest of the records are all homogeneous and of known size. That allows to load them into memory without the need for expensive parsing.
