...
- Serializing them to text and concatenating them using the non-ASCII byte separator `1D`.
- Computing the `xxh32` hash and returning its most significant byte.

TODO(miguel): Document the false positive and false negative rates of column reporting. Also the max column cap before giving up and reporting the whole row as altered to trade false positives for false negatives. All this based on the exploration of a single dataset.
