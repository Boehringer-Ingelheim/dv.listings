...
- `allow_row_deletion`: `[logical(1)]` If FALSE (default), the review feature will guard against dataset updates that remove previously known rows (as identified by the `id_vars` subfield). This check can be disabled by setting `allow_row_deletion` to TRUE.
