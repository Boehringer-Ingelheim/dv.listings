...
Please note that **this feature is still experimental and may be subject to change**. Also note that, at the moment, **only 
one instance of the `dv.listings` module can provide this feature on any given DaVinci app**.
...
- A "bulk review" menu (top-left) that allows users to:
  ...
  7. See a description of the latest review action (either single or bulk) for the current role and undo it.
...
As a study progresses, it is possible for data that has already been reviewed to change. In order to bring those changes to the attention of reviewers, `dv.listings` automatically highlights altered data cells (on columns specified as `tracked`) that may make previous reviews outdated.
...
- `allow_row_deletion`: `[logical(1)]` If FALSE (default), the review feature will guard against dataset updates that remove previously known rows (as identified by the `id_vars` subfield). This check can be disabled by setting `allow_row_deletion` to TRUE. Reversing this action is not supported at the moment.
...
This is the first DaVinci module that stores permanently a substantial amount of study decisions taken over an extended period of time. Loss of data in this context is not acceptable. To guarantee data integrity, we propose the use of simple *append-only* data structures. Those are stored as plain files opened in "append" mode. It's easy to demonstrate that they can't become the source of catastrophic loss of data of past review sessions. The granularity of the `pins` API (which only allows to load and store complete files) is incompatible with this design. 
...
<b>New in version 1 of the `.review` format:</b>

Reviews can be undone. Their records are not erased from the `.review` file. Instead, new "undo" records are appended
indicating that the original actions should have no effect. The format of an undo action record is the following:

- `[4-byte integer]` `0` (undo action marker)
- `[4-byte integer]` amount of actions to undo (1 for single review, >1 for bulk review)
...
Review and undo records coexist inside the same file. If the first four bytes of a record are zero, the reader knows to
interpret it as 32-byte (double-width) undo record. The last two elements of an undo record are used as a redundancy check
against the original target action.
  
This representation is backwards-compatible with that of version 0, as it imbues the unused zero value of the first field with
new meaning.

This representation _is not_ forwards compatible. Older versions of the `dv.listings` module are not capable of 
interpreting undo actions, and they will refuse to load this new file format.
</details>
...
A popular choice for non-cryptographic hashing is the `xxh` family of function (https://xxhash.com/) for its balance of speed and perceived hash quality.
