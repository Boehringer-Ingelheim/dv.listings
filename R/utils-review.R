  ...
  for (dataset_lists_name in names(dataset_lists)) {
    ...
    for (dataset_review_name in names(review[["datasets"]])){
      ...
      if (file.exists(fname)) {
        ...
        contents <- RE_compute_base_memory(dataset_review_name, dataset, id_vars, tracked_vars)
      ...
      if (file.exists(fname)) {
        ...
        contents <- RE_compute_review_codes_memory(review[["choices"]])
        writeBin(contents, fname)
      }
      ...
      for (role in review[["roles"]]){
        ...
        if (file.exists(fname)) {
          ...
          contents <- RE_compute_review_reviews_memory(role, dataset_review_name)
          writeBin(contents, fname)
        }
        
        # NOTE: each role keeps their own decisions...
        role_review <- RE_parse_review_reviews(contents, row_count = nrow(dataset), 
...
REV_logic_2 <- function(ns, state, input, review, datasets, selected_dataset_list_name, selected_dataset_name, data,
  ...
  shiny::observeEvent(input[[REV$ID$REVIEW_SELECT]], {
    ...
    contents <- c(
      SE$integer_to_raw(i_row),
      SE$integer_to_raw(option),
      SE$double_to_raw(SE$get_UTC_time_in_seconds())
    )
